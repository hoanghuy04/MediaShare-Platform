...
public abstract class UserMapper {
    ...
    public UserProfile getProfileWithAvatarUrl(UserProfile userProfile) {
        ...
        try {
            ...
            log.warn("Error fetching avatar URL for path: {}, returning profile with null avatar", avatarPath);
            // Return profile with null avatar on error instead of keeping invalid path
            UserProfile clonedProfile = UserProfile.builder()
                    .firstName(userProfile.getFirstName())
                    .lastName(userProfile.getLastName())
                    .bio(userProfile.getBio())
