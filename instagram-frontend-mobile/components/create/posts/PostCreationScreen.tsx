...
export default function PostCreationScreen({ onClose, onStepChange, onPostCreated }: PostCreationScreenProps) {
  ...
  const handleTakePhoto = useCallback(async () => {
    try {
      ...
      if (photo) {
        // Lưu ảnh vào MediaLibrary để có thể load lại
        try {
          await MediaLibrary.saveToLibraryAsync(photo.uri);
        } catch (saveErr) {
          // Ignore nếu không lưu được (có thể đã có trong library)
        }
        
        // Refresh gallery để load ảnh mới chụp
        await loadGallery();
        
        // Tìm ảnh vừa chụp trong gallery (ảnh mới nhất)
        // Vì ảnh mới chụp sẽ ở đầu danh sách sau khi refresh
  ...
  const goNext = () => {
    ...
    router.push({
      pathname: '/create/posts/edit-media',
      params: {
